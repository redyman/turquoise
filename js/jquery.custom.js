$(document).ready(function(){
    ...
    if ( $(window).width() > 767 ) {
		...
		$('#seccion-works .item').fadeTo(0, 0.8);

		// Fade in and out on hover in/out
		$('#seccion-works .item').hover(function() {
	...
	// Wrap images in #seccion-works to remove spacing at the bottom
	// the wrapper div needs to have line-height: 0
	$('#seccion-works img').wrap('<div class="img-wrap" />');
...
$(window).resize(function () {
	...
	var profileImageHeight = $('#seccion-profileImage').height();
    ...
    if ( windowWidth > 767 ) {
		...
		$('#seccion-works .item').fadeTo(0, 1);

		// Unbind hover effect
		$('#seccion-works .item').unbind('mouseenter mouseleave');
    }
